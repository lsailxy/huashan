...
<template>
  <div class="I106_page">
    <div class="I106_header">
      ...
      <div class="I106_title">{{pointName}}</div>
    ...
    <div class="H106_content">
      <list :listData="listData" @update="updateList" ref="electricityDeviceList"></list>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    pointName() {
      return this.$route.query.pointName
    }
  },
  // 组件挂载
  components: {
  ...
  methods: {
    ...
    async updateList(currentPage) {
      ...
      const res = await electricity.jumpEquipmentList(json)
      if(res && res.status === 10001) {
        ...
        this.$refs.electricityDeviceList.isAllLoad(res.result.total)
      } else {
        this.$refs.electricityDeviceList.errorHandle()
      }
    },
    /**
     * 页面跳转
