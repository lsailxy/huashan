...
<template>
  <van-pull-refresh v-model="loading" @refresh="initData()">
    ...
    >
      <ul class="H106_list">
        <li v-for="(item, index) in listData" :key="'enterprisePoint_'+index" @click="jumpPage('electricityDevice', {pointId: item.id}, {enterpriseName: $route.query.enterpriseName, pointName: item.name})">
          <div class="E106_listItem">
            <div class="E106_listItemName">
              <div>{{item.name}}</div>
              <div class="E106_place">地址：{{item.place}}</div>
            </div>
