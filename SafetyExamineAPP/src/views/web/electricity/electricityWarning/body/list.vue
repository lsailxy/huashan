...
<template>
  <van-pull-refresh v-model="loading" @refresh="initData()">
    <van-list
      ...
      @load="changeList()"
    >
      <ul class="E406_warningList">
        <li v-for="(item, index) in listData" :key="'enterprisePoint_'+index">
          <div class="E406_warning clear">
            <div class="E406_warningPlace">{{item.place}}</div>
            <div class="E406_warningTime">{{item.time}}</div>
            <div class="E406_warningType">{{item.type}}</div>
          </div>
        </li>
      </ul>
    </van-list>
...
<style lang="scss" type="text/scss" scoped>
    ...
    .E106_place {font-size: val(14); color: #666666;}
    .E406_warningList {padding-bottom: 1rem;}
    .E406_warningList>li {margin-top: 1rem; background-color: #ffffff;}
    .E406_warning {position: relative; padding: 1rem;}
    .E406_warningPlace {color: #010101; font-size: 1.6rem; padding: 0.5rem 0; max-width: 65%; word-break: break-all;}
    .E406_warningTime {color: #546a79; font-size: 1.4rem; padding: 0.5rem 0;}
    .E406_warningType {position: absolute; right: 1rem; top: 1rem; font-size: 1.5rem; color: #ffffff; background-color: #e10b13; height: 2.6rem; line-height: 2.6rem; border-radius: 2.6rem; padding: 0 1rem; box-shadow: 0.3rem 0.3rem 0.3rem rgba(225,11,19,.3);}
</style>
