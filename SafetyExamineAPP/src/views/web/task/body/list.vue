...
<template>
  <van-pull-refresh v-model="loading" @refresh="initData()">
    ...
    >
      <ul class="H106_list">
        <li v-for="(item, index) in listData" :key="'task_'+index" @click="jumpPage2(item)">
              ...
              <div class="H106_item">
                <div class="H106_itemDatas">
                  <div class="H106_itemData">
                    <span>巡查人：</span>
                    <span>{{item.patrolusername}}</span>
                  </div>
                  <div class="H106_itemData">
                    <span>未检查企业数量：</span>
                    <span class="H106_itemNumber">{{item.patrolCount || 0}}</span>
                  </div>
                  <div class="H106_itemData">
                    <span>未回头看数量：</span>
                    <span class="H106_itemNumber">{{item.unreviewedCount || 0}}</span>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    jumpPage2(data) {
      if(data.type === 1) {
        this.jumpPage('accompanyingDetails', { taskid: data.id, taskdetailid: data.taskdetailid, isCheck: data.taskstatus === '1' ? 0 : 1 }, { taskName: data.name })
      } else {
        this.jumpPage('taskDetails', { taskid: data.id })
      }
    },
    /**
     * 删除计划请求
     ...
     * @param index 下标
     */
    async delTaskAjax(item, index) {
      let json = {
        id: item.id
      }
      const res = await task.deleteTask(json)
...
<style lang="scss" type="text/scss" scoped>
    ...
    .H106_list {}
    .H106_list>li {}
    .H106_itemOuter {border-bottom: 1px solid #e9e9e9; padding: val(12) val(12) val(12) val(21);}
