...
<template>
  <div class="I106_page">
    ...
    <div class="H106_content">
      <list :listData="listData" @update="updateList" ref="reportList"></list>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async updateList(currentPage) {
      ...
      const res = await task.reportlist(json)
      if(res && res.status === 10001) {
        if(currentPage > 1) {
        ...
        this.$refs.reportList.initPDF()
        this.$refs.reportList.isAllLoad(res.result.total)
      } else {
        this.$refs.reportList.errorHandle()
      }
    },
    /**
     * 页面跳转
...
<style lang="scss" type="text/scss" scoped>
  ...
  .H106_content {overflow: auto; height: 100%; padding-top: val(79); padding-bottom: val(55); background-color: #f2f2f2;}
