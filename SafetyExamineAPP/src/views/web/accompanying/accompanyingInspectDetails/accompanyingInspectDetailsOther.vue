  ...
  <div class="I106_page">
    ...
    <div class="H106_content">
      <div class="I206_listOuter">
        ...
        <ul class="I206_list">
          <li v-for="(item, index) in formList" :key="'faillist_'+index">
            <div>
              <div class="H106_checkSign">
                <img src="@/assets/images/H106_sign1.png" v-show="item.reviewcheckvalue === 1">
                <img src="@/assets/images/H106_sign2.png" v-show="item.reviewcheckvalue === 2">
              </div>
            </div>
            <div v-if="isCheck==1" class="I206_itemName">{{item.name}}</div>
            ...
            <div class="I206_itemBtnOuter" v-if="isCheck==0">
              ...
              <div class="I206_itemBtn">
                <el-popover
                  ref="popover"
                  placement="bottom"
                  v-model="checkVisibles['check_'+index]"
                  trigger="click">
                  <ul class="S106_more H106_check">
                    <li @click="isCheckbuttonvalue(index,item.reviewcheckvalue,1)">
                      <img src="@/assets/images/R106_icon2.png" alt="">
                      <span>已合格</span>
                    </li>
                    <li @click="isCheckbuttonvalue(index,item.reviewcheckvalue,2)">
                      <img src="@/assets/images/R106_icon3.png" alt="">
                      <span>不合格</span>
                    </li>
                  </ul>
                  <img class="I106_icon" slot="reference" src="@/assets/images/R106_icon1.png" alt="">
                </el-popover>
              </div>
            </div>
          </li>
...
export default {
  ...
  methods: {
    async initData() {
      const res = await common.getTypeAndLevel()
      if(res && res.status === 10001) {
        this.dicts.levels = res.result.levellist || []
        this.dicts.types = res.result.typelist || []
        this.dicts.smalltypes = res.result.smalltypelist || []
        ...
        this.formList.forEach((item, index) => {
          ...
          if(item.selfpatrolurls) {
            item.selfpatrolurls.forEach((item2) => {
    ...
    async delFromAjax(index) {
      if(this.formList[index].id) {
        ...
        let delRes = await inspect.delOtherSelfPatrolinfo(del)
    ...
    async newForm() {
      ...
      let res = await inspect.addOtherSelfPatrolinfo(json)
      if(res && res.status === 10001) {
        this.formList.unshift(this.createJson(res.result.id))
    ...
    createJson(id, name, inputval, checkbutton, selfpatrolurls, type, level, place, typename, levelname, smalltype, smalltypename) {
      let json = {
        ...
        checkbuttonvalue: checkbutton || 2,
    ...
    async saveData() {
      ...
      for(let i = 0; i < this.formList.length; i++) {
        arr.push(this.createJson(this.formList[i].id, this.formList[i].name, this.formList[i].inputcontent, this.formList[i].checkbuttonvalue, this.formList[i].selfpatrolurls, this.formList[i].types.inputValue.split(',')[0], this.formList[i].levels.inputValue, this.formList[i].place, this.formList[i].types.inputLabel.split('-')[0], this.formList[i].levels.inputLabel, this.formList[i].types.inputValue.split(',')[1], this.formList[i].types.inputLabel.split('-')[1]))
        if(!arr[i].name) {
...
<style lang="scss" type="text/scss" scoped>
  ...
  .I206_list>li {padding-left: val(10); background-color: #ffffff; margin-top: val(10); position: relative;}
  ...
  .H106_check li>img {width: 2rem; height: 2rem; margin-right: 1rem; vertical-align: middle;}
  .H106_check li>span {display: inline-block; width: 5rem; text-align: center;}
  .I106_icon {width: val(21); vertical-align: middle;}
  .H106_checkSign {position:absolute;top: val(12);right: val(12);}
  .H106_checkSign img {height: val(60);}
</style>
