...
<template>
  <van-pull-refresh v-model="loading" @refresh="initData()">
    ...
    >
      <ul class="H106_list">
        <li v-for="(item, index) in listData" :key="'enterprise_'+index" @click="addAccompanyTask(item)">
          <div class="E106_listItem">
            <div class="E106_listItemName">{{item.name}}</div>
...
export default {
  ...
  watch: {},
  methods: {
    addAccompanyTask(data) {
      this.$dialog.confirm({
        message: '确定检查“' + data.name + '”？'
      }).then(() => {
        this.addAccompanyTaskAjax(data)
      }).catch(() => {})
    },
    async addAccompanyTaskAjax(data) {
      let json = {
        enterprise: data.enterpriseid,
        enterprisename: data.name,
        enterprisestatus: data.type,
        depname: JSON.parse(localStorage.getItem('userInfo')).JGMC,
        organizationtype: JSON.parse(localStorage.getItem('userInfo')).JGXZCODE
      }
      ...
      if(res && res.status === 10001) {
        ...
        this.$router.replace({
          ...
          params: {
            ...
            taskdetailid: res.result.taskdetailid,
            isCheck: 0
          },
          query: {
            enterpriseName: data.name
