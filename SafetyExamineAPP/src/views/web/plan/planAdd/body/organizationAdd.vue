        ...
        <div class="H106_content">
          <van-pull-refresh v-model="loading" @refresh="updateList(1)">
            <van-list
              ...
              @load="changeList()"
            >
              <div @click="toggle(index)" v-for="(item, index) in searchResult" :key="'checkbox_'+type+index">
                <van-checkbox
                  v-model="item.checked"
                  :name="item.id"
                  checked-color="#16a35f"
                  ref="checkboxes"><span v-html="brightenKeyword(item.jgmc, searchValue)"></span></van-checkbox>
              </div>
            </van-list>
          </van-pull-refresh>
        </div>
        <div class="E206_resultOuter">
          <div class="E206_resultNumber" @click="resultShow">已选择{{result.length}}家机构(点击查看)</div>
            ...
            <div class="H106_content H106_content2">
              <div class="E206_resultList">
                <div class="E206_resultItem" v-for="(item, index) in result" :key="'result_'+index">
                  <div class="E206_resultItemName">{{item.jgxzname}}-{{item.jgmc}}</div>
...
export default {
  ...
  methods: {
    showPicker() {
      ...
      this.searchResult = this.listData
      this.popupShow = true
    },
    cancelChoose() {
    ...
    search() {
      this.searchResult = this.searchList(this.searchValue, this.listData)
    },
    /**
     * 数组模糊搜索
     ...
     */
    async initData() {
      const res = await plan.toPlanAdd()
    ...
    toggle(index) {
      if(!this.searchResult[index].disabled) {
        if(!this.searchResult[index].checked) {
          this.result.push(this.searchResult[index])
        } else {
          this.result.forEach((item2, index2) => {
            if(item2.id === this.searchResult[index].id) {
              this.result.splice(index2, 1)
            }
          })
        }
        this.searchResult[index].checked = !this.searchResult[index].checked
    ...
    async updateList(currentPage) {
      if(this.type) {
        ...
        const res = await plan.getDepname(json)
        if(res && res.status === 10001) {
          ...
          this.searchResult.forEach((item1, index1) => {
            this.result.forEach((item2, index2) => {
              if(item1.id === item2.id) {
                item1.checked = true
              }
